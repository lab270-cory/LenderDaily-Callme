...
class TimezoneSeeder extends Seeder
{
    public function run()
    {
        foreach ($this->getTimezoneList() as $displayName => $name){
            Timezone::updateOrCreate(['name'=> $name, 'display_name'=> $displayName]);
        }
    ...
    /**
     * Updates old availabilities' timezone
     */
    public function updateAvailabilities()
    {
        $serverTimezone = (new \DateTime())->getTimezone()->getName();
        $defaultTimezoneId = Timezone::where('name', $serverTimezone)->value('id');

        User::all()->map(function($element) use ($defaultTimezoneId){
            $element->timezone_id = $defaultTimezoneId;
            $element->save();
        });
    }
